...
Deliverables:
1. First, extract can_access_party(p_party_id, p_user_id) as a helper and
   REFACTOR the existing parties RLS policy to use it. Do this before adding
   anything new — the feed must not reimplement the visibility rule, and
   Phases 5 and 6 will reuse the same helper. One implementation, one place
   to change.
